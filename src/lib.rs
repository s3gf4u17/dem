...
pub fn sphere(a:f64,b:f64,file:&str) -> Vec<(f64,f64,f64)> {
    ...
    for i in 0..raster_height*raster_width {
        ...
        let h = (pixel[0] as f64 +pixel[1] as f64 +pixel[2] as f64)/3.0;
        coordinates.push((phi,lambda,h));
    }
    // transform coordinates to (x y z)
    for i in 0..coordinates.len() {
        let (phi,lambda,h) = coordinates.get(i).unwrap();
        let n = a/libm::sqrt(1.0-e2*libm::sin(*lambda)*libm::sin(*lambda));
        let x = (n+*h)*libm::cos(*lambda)*libm::cos(*phi);
        let y = (n+*h)*libm::cos(*lambda)*libm::sin(*phi);
        let z = (n*(1.0-e2)+*h)*libm::sin(*lambda);
        coordinates[i]=(x,y,z);
    }
    coordinates
